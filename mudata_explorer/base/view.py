...
class View(MuDataAppHelpers):
        ...
        for key, elem in schema.items():
            ...
            if elem["type"] == "object":
                ...
                if elem.get("query", True):
                    for attr in ["modality", "table", "cname", "expr", "value"]:
                        yield (
    ...
    def render_dataframe(
        ...
        if elem.get("select_columns", False):
            ...
            if any([cname not in avail_columns for cname in selected_columns]):
                selected_columns = [cname for cname in selected_columns if cname in avail_columns]
            ...
            if settings["editable"]:
                ...
                # Otherwise, present a list of columns to select from
                    ...
                    container.multiselect(
                        "Select columns",
                        avail_columns,
                        **self.input_multiselect_kwargs(selected_columns_kw, avail_columns)
                    )
    ...
    def render_query(self, prefix: str, key: str, container: DeltaGenerator):
        ...
        try:
            ...
            container.write(f"Error while filtering")
